...
| Boissonnat et al., *Triangulations in CGAL* | The separation of geometric predicates from combinatorial storage maps to `hyperlimit` versus `TriangulationDataStructureD`. Stable handles, opposite-facet neighbor slots, canonical facets, and explicit validators already follow the paper's design. Reusing certified builder facts is the retained implementation consequence. |
